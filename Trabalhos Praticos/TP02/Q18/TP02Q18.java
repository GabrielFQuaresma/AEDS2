...
class Jogador {
    ...
    public static int statecmpr(Jogador jog1, Jogador jog2)
    {
        return jog1.getEstadoNascimento().compareTo(jog2.getEstadoNascimento());
    }
...
class Queue{
    ...
    public void show(int k) throws Exception
    {
        if(k > size)
        {
            throw new Exception("ERROR: Partial is greater then the size of queue");
        }
...
// -------------------------------------------- Ordenação --------------------------------------------
    ...
    private void RecursiveQuickSortPartial(int k, int esq, int dir)
    {
        ...
        int i = esq;
        int j = dir;
        while ((Jogador.statecmpr(array[i], pivo)) < 0 || ((Jogador.statecmpr(array[i], pivo) == 0) && (Jogador.strcmpr (array[i], pivo)) < 0)) i++;
        while ((Jogador.statecmpr(array[j], pivo)) > 0 || ((Jogador.statecmpr(array[j], pivo) == 0) && (Jogador.strcmpr(array[j], pivo)) > 0)) j--; //Desempate por nome 
...
public class TP02Q18 {
    ...
    {
        String fileName = "814832_partialselecao.txt";
