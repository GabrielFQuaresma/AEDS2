...
void countingSort( Queue *queue, Resultado* resultado)
{
    Jogador* array = queue->array;
    int size = queue->size;
    ...
    if (size <= 0)
    {
        printf("Erro: Fila com tamanho invalido");
        exit(-1);
    }

    int maxValue = queue->array[0].id;
    int minValue = queue->array[0].id;

    for (int i = 1; i < size; i++)
    {
        maxValue = (maxValue < array[i].id) ? array[i].id : maxValue;
        minValue = (minValue > array[i].id) ? array[i].id : minValue;
    }
    int diff = maxValue - minValue;


    int* countArray = (int*) calloc(diff + 1, sizeof(int)); //inicialização do vetor intermediário

    for(int i = 0; i < size; i++)
    {
        int index = array[i].id - minValue;
        countArray[index]++;
    }

    Queue exitQueue = newQueue(size);

    for(int i = 1; i < maxValue + 1; i++) countArray[i] += countArray[i-1]; //Somando os valores de trás

    for(int i = 0; i < size; i++)
    {
        int position = array[i].id - minValue;
        exitQueue.array[--countArray[position]] = array[i];
...
void registroLog(Timer timer, Resultado resultado)
{
    ...
    literal fileName = "814832_radixsort.txt";
...
int main()
{
    ...
    {
        Jogador tmp = readJogador(atoi(line), "/workspaces/AEDS2/Trabalhos Praticos/TP02/Q14/tmp/players.csv");
        queue.insert(tmp, &queue);
        scanf("%s", line);
        getchar();
    }
