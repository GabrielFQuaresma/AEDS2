...
void registroLog(Timer timer, Resultado resultado) {
	...
	literal fileName = "814832_shellsort.txt";
...
int main ()
{
    ...
    {
        Jogador tmp = readJogador(atoi(line), "/workspaces/AEDS2/Trabalhos Praticos/TP02/Q08/tmp/players.csv");
        queue.insert(tmp, &queue);
        scanf("%s", line);
        getchar();
    }
    ...
    KnuthShellSortWeigth(&queue, &resultado);
