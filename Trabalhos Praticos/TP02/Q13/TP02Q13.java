...
class Jogador {
    ...
    public static int unicmpr(Jogador jog1, Jogador jog2)
    {
        return jog1.getUniversidade().compareTo(jog2.getUniversidade());
    }
...
class Queue{
    ...
    public void insert(int position, Jogador jogador) throws Exception
    {
        if(size == maxSize || position > maxSize || position < 0){
            throw new Exception("Erro: Inserindo incorretamente");
        }

        this.array[position] = jogador;
        size++;
    }
...
// -------------------------------------------- Ordenação --------------------------------------------
    ...
    public ResultadoPesquisa countingSort() throws Exception{
        if (size <= 0) {
            throw new Exception("Erro: Fila com tamanho inválido");
        }
        ...
        for(int i = 1; i < size; i++)
           maior = (maior < array[i].getAltura()) ? array[i].getAltura() : maior;

        int[] countArray = new int[maior + 1];

        //Fazendo a contagem em si
        for(int i = 0; i < size; i++){
            int index = array[i].getAltura();
            countArray[index]++;
        }
        
        //Somando o numero para achar a posição atual
        for(int i = 1; i < countArray.length; i++)
            countArray[i] += countArray[i - 1];
        
        Queue exitQueue = new Queue(size); //Uma nova fila do mesmo tamanho

        for(int i = 0; i < size; i++)
        {
           int index = array[i].getAltura();
           exitQueue.insert(--countArray[index], array[i]);
            
           //Jogador.print(array[i]);
           resultado.incrementarMovimentacoes(1);
        }
        array = exitQueue.array;
        return resultado;
    }
...
// ----------------------------------------- MergeSort -----------------------------------------
    ...
    private void merge(int left, int right,  ResultadoPesquisa resultado) {
		
		int auxSize = (right - left) + 1, mid = ((auxSize + 1) / 2);

	    Jogador[] array = this.array;
		Jogador[] auxArray = new Jogador[auxSize];

		for (int i = 0; i < auxSize; i++) auxArray[i] = array[left + i];
		resultado.incrementarMovimentacoes(auxSize);

		for (int i = 0, j = mid; left <= right; left++) {
			if (i < mid && j < auxSize) {
				boolean comp = ((Jogador.unicmpr(auxArray[i], auxArray[j])) < 0) || ((Jogador.unicmpr(auxArray[i], auxArray[j])) == 0 && Jogador.strcmpr(auxArray[i], auxArray[j]) < 0);
    ...
    private ResultadoPesquisa RecursivemergeSort(int left, int right, ResultadoPesquisa resultado)
    {

        if (left < right) {
			int mid = (left + right) / 2;
			RecursivemergeSort(left, mid, resultado);
			RecursivemergeSort(mid + 1, right,  resultado);
			merge(left, right, resultado);
		}
        return resultado;
    }

    public ResultadoPesquisa mergeSort()
    {   
        return (RecursivemergeSort(0, size - 1,new ResultadoPesquisa()));
    }
}
...
class ResultadoPesquisa {
    ...
    public void setComparacoes(int comparacoes) { this.comparacoes = comparacoes; }
...
public class TP02Q13 {
    ...
    {
        String fileName = "814832_mergeSort.txt";
    ...
    public static void main(String[] Args)
    {
        ...
        {
            System.out.println(e.getMessage() + e.getLocalizedMessage() + e.getClass());
        }
