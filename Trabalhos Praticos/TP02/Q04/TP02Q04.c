...
void RecursiveSelectionSort(int i, int j, int menor, Resultado* resultado, Queue* queue)
{
    if(j < queue->size)
    {
        ...
        RecursiveSelectionSort(i, j + 1, menor, resultado, queue);
    }
    else{
    ...
    {
        RecursiveSelectionSort(i, i + 1, i, resultado, queue);
    }
}

void SelectionSort(Resultado* resultado, Queue* queue)
{
    RecursiveSelectionSort(0, 0, 0, resultado, queue);
}

bool RecursiveBinarySearch(int left, int right, String name, Resultado* resultado, Queue queue)
{
    ...
    if(strstr(tmp.nome, name))
        ...
        right = middle - 1;
    resultado->comparacoes++;
    return left <= right ? RecursiveBinarySearch(left, right, name, resultado, queue) : false;
}

bool BinarySearch(String name, Resultado* resultado, Queue queue)
{
    return RecursiveBinarySearch(0, queue.size - 1, name, resultado, queue);
}
...
void registroLog(Timer timer, Resultado resultado) {
	...
	literal fileName = "814832_binaria.txt";
...
int main ()
{
    ...
    {
        Jogador tmp = readJogador(atoi(line), "/workspaces/AEDS2/Trabalhos Praticos/TP02/Q04/tmp/players.csv");
        queue.insert(tmp, &queue);
        scanf("%s", line);
        getchar();
    }   
    ...
    SelectionSort(&resultado, &queue);
    
    timer.Start(&timer);
    while(strcmp(line, "FIM"))
    {
        printf(BinarySearch(line, &resultado, queue) ? "SIM\n" : "NAO\n");
        scanf("%[^\n]",line);
        getchar();
    }
    timer.Stop(&timer);
